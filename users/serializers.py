...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if user.auth_type == VIA_EMAIL:
            ...
            # send_phone_code(user.email, code)
        user.save()
        return user
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                'success':False,
                'message': "Bu email ma'lumotlar bazasida bor"
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                'success': False,
                'message': "Bu telefon raqami ma'lumotlar bazasida bor"
            }
            raise ValidationError(data)
        return value
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if password != confirm_password:
            raise ValidationError(
                {
                    'message':"Parol va tasdiqlash paroli bir-biriga mos emas!"
                }
            )
        if password:
    ...
    def validate_username(self, username):
        if len(username) <5 or len(username) > 30:
            raise ValidationError(
                {
                    'message': "Username must between 5 and characters long"
                }
            )
        if username.isdigit():
            raise ValidationError(
    ...
    def validate_first_name(self, first_name):
        if len(first_name) <3 or len(first_name) > 30:
            raise ValidationError(
                {
                    'message': "First_name must between 3 and characters long"
                }
            )
    ...
    def validate_last_name(self, last_name):
        if len(last_name) <3 or len(last_name) > 30:
            raise ValidationError(
                {
                    'message': "Last_name must between 3 and characters long"
                }
            )
        if last_name.isdigit():
            raise ValidationError(
                {
                    'message': "This last_name is entirely numeric"
                }
            )
        return last_name
    ...
    def auth_validate(self, data):
        ...
        if user is not None:
            ...
            raise ValidationError(
                {
                    'success': False,
                    'message': "Kechirasiz login yoki parol xato! Iltimos boshqatdan urinib ko'ring"
                }
            )
    def validate(self, data):
